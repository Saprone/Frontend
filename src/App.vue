...
export default {
  ...
  methods: {
    async fetchBasket() {
      try {
        const response = await axios.get('http://localhost:8083/basket');
    ...
    async getRecipeStatus() {
      try {
        await axios.get('http://localhost:8084/recipes/status');  
    ...
    async fetchRecipes() {
      ...
      try {
        const response = await axios.get('http://localhost:8084/recipes');  
    ...
    async addIngredientToBasket(ingredient) {
      if (!this.basket.find(item => item.id === ingredient.id)) { 
        ...
        await axios.post('http://localhost:8083/basket/ingredient/add', JSON.stringify(ingredient));  
    ...
    async removeIngredientFromBasket(index) {
      ...
      try {
        await axios.post('http://localhost:8083/basket/ingredient/remove', ingredientId);
